...
function Toolbar({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  return (
    <div className="flex flex-wrap gap-1 border-b border-white/10 bg-white/5 p-2">
      ...
      <button type="button" onClick={() => editor.chain().focus().setParagraph().run()} className={btn(editor.isActive("paragraph"))}>
        P
      </button>
      ...
      <button type="button" onClick={() => editor.chain().focus().setHorizontalRule().run()} className="px-2 py-1 rounded text-sm text-white/60 hover:bg-white/10">
        HR
      </button>
    </div>
  );
